...
GDUP 默认将数十 GB 的引擎本体解压在 \~/.gdup/versions\ 中。对于 C 盘空间紧张的 Windows 用户，可自行在系统中添加 \GDUP_CACHE_PATH\ 环境变量，程序会自动将引擎安装到该路径下的 \ersions\ 目录中。
...
项目已集成 GitHub Actions 流水线。如需发布新版本，只需打上以 \\ 开头的 Tag 并推送到远端：
