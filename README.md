...
GVM 是一个专为 **Godot Engine** 设计的轻量级、无痛且绿色的版本管理器。它采用“透明包装器（Wrapper）”的设计理念，将自身命名为 `godot`（或 Windows 下的 `godot.exe`）。当您执行任何非管理命令时，它会根据当前目录或全局配置，自动且透明地将命令与参数转发给对应的真实 Godot 版本。

---

## 🌟 核心特性

- **透明转发（Transparent Forwarding）**：作为 Godot 启动的拦截器。在日常开发中，您可以像直接使用官方 Godot 一样输入 `godot --editor` 或 `godot -e`，GVM 会根据项目配置自动调用正确的版本，完全不改变原有的开发习惯。
- **目录级版本隔离**：支持通过项目根目录下的 `.gvmrc` 配置文件为不同项目指定不同的 Godot 版本。若当前目录没有配置，它会沿着目录树向上递归查找，或最终匹配用户家目录中的全局 `.gvmrc`。
...
GVM 核心是一个透明代理，其工作逻辑如下：

```mermaid
graph TD
    A[执行 godot 命令] --> B{是否为 GVM 管理命令? <br> install/uninstall/use/list/releases...}
    B -- 是 --> C[执行 GVM 管理逻辑]
    B -- 否 --> D[在当前目录向上递归查找 .gvmrc]
    D --> E{是否找到 .gvmrc?}
    E -- 否 --> F[查找家目录 ~/.gvmrc]
    E -- 是 --> H[解析目标 Godot 版本]
    F --> G{是否找到 ~/.gvmrc?}
...
### 3. 版本存储目录
GVM 会在包装器所在的同级目录下，自动创建并维护一个名为 `godot-versions/` 的文件夹，所有下载的 Godot 官方版本均会存放于此：
```text
your-tools-path/
├── godot.exe                   # GVM 包装器二进制文件
...
### 2. 下载并安装指定版本
指定版本号进行安装，GVM 会自动选择适合当前系统架构的官方包下载并解包：
```bash
# 安装 4.2.2 稳定版
godot install 4.2.2
...
### 3. 配置当前项目的 Godot 版本
在您的 Godot 项目根目录下执行 `use` 命令，这会在当前目录生成一个 `.gvmrc` 配置文件：
```bash
# 将当前项目指定为使用 4.2.2 版本
godot use 4.2.2
```
`.gvmrc` 文件是一个简单的 JSON 配置：
```json
{
  "version": "4.2.2"
}
...
### 5. 启动 Godot
在配置了 `.gvmrc` 的项目目录下，您可以直接运行以启动 Godot 编辑器或执行命令行指令：
```bash
...
```
*注：`-H windowsgui` 参数用于剥离默认的 Windows 控制台窗口，避免您直接运行 `godot` 启动编辑器时弹出一个空白的 Cmd 黑框。GVM 在内部通过 API 实现了有无控制台环境的智能适配。*
